...
class retrievedata_model extends User_Model {
    ...
    function rideTickerQuery($radius = 25) {
        $query = $this->searchRideByRadiusQuery($this->coords, 
                $radius, 5);

        return $this->db->retrieveRows($query);
    }
    
    function returnRides($lat, $lon, $radius = 25, $limit = 100) {
        $coord = array($lat, $lon);
     ...
     function retrieveEventInformation($id) {
         $query =  "SELECT "
                    . "e.Name, e.Address, e.City, e.State, e.Zip, "
